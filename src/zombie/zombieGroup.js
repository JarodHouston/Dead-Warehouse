...
export class ZombieGroup {
    constructor(numZombies, wallMatrix, playerPosition, scene, radius = 0.5) {
      ...
      this.radius = radius;
      this.zombies = [];
      this.playerTile = {x: Math.floor(this.playerPosition.x), y: Math.floor(this.playerPosition.z)};
      this.attackRate = 1;
    ...
    animate(elapsed,dt) {
      // const damage = 0;
        ...
        this.zombies.forEach(z => {   
            z.animate(elapsed, dt, tileChange && !this.isWall(this.playerTile.x, this.playerTile.y));
            // if (Math.round((z.attackTime % this.attackRate) * 10)/10 == Math.round((elapsed % this.attackRate))) damage++;
        });
        // return damage;
    }
    updatePlayerTile() {
      const ret = (Math.floor(this.playerPosition.x) != this.playerTile.x || Math.floor(this.playerPosition.z) != this.playerTile.y);
