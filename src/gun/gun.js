...
import {
  REST_Z, KICK_Z, RECOVER_MS
} from '../core/constants.js';

let weaponAnchor, controls;

//const loader = new GLTFLoader();
let recoilClock   = null;  // NEW  – null when idle
...
/* ── helpers ──────────────────────────────────────────────────────────────── */
function quadOut(t) {     // quadratic-out easing (t in [0, 1])
  return 1 - (1 - t) * (1 - t);
}

/* ── animation loop hook (runs every frame) ───────────────────────────────── */
export function updateRecoil() {                         // NEW
  ...
  weaponAnchor.position.z = THREE.MathUtils.lerp(
    recoilStartZ,
    REST_Z,
    eased
  );
