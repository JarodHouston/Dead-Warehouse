...
export function startGameLoop({
  ...
  function animate() {
    ...
    for (let i = 0; i < SUBSTEPS; i++) {
      handleInput(step, getKeys(), velocity, camera, onFloor, walkSound, sprintSound);
      onFloor = playerPhysics(playerCollider, velocity, worldOctree, spawnPos, step);
    }
    ...
    updateGunRecoil();            // from gun/gun.js (visual recoil)
    updateRecoil();               // user‑supplied additional recoil (if any)
    updateZombie(clock.getElapsedTime());
